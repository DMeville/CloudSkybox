...
Shader "CloudSkybox"
{
	Properties
	{
		...
		[HDR]
		_CloudTintShadow("Cloud Shadow Tint", Color) = (.5, .5, .5, 1)
		[HDR]
		_CloudTintHighlight("Cloud Highlight Tint", Color) = (.5, .5, .5, 1)
	}
...
#include "ProceduralSky.cginc"
		...
		float4 _CloudTintHighlight;
		float4 _CloudTintShadow;
		...
		fixed4 frag(v2f i) : SV_Target
		{
			...
			acc = lerp(_CloudTintShadow, _CloudTintHighlight, saturate(acc))*sky;
			acc += Beer(depth) * sky;
